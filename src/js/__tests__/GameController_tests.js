...
const posSwordman = new PositionedCharacter(new Swordman(1), 1);
...
testCtrl.gameState.allPositions.push(posBowman, posSwordman, posVampire, posDaemon);
...
test('метод addsTheTeamToPosition должен добавить команду в gameState.allPositions', () => {
  ...
  userTeam.addAll([bowman, swordman]);
...
test('метод getChar должен вернуть персонажа по индексу', () => {
  expect(testCtrl.getChar(1)).toEqual(posSwordman);
