...
export default class GameController {
  ...
  getBotsResponse() {
    ...
    const usersTeam = this.gameState.allPositions.filter((e) => (
      ...
      || e.character instanceof Swordman
    ...
    if (target) {
      const botChar = bot.character.attack;
      const damage = Math.max(botChar - target.character.defence, botChar * 0.1);
  ...
  onLoadGameClick() {
    ...
    load.allPositions.forEach((elem) => {
      ...
      switch (elem.character.type) {
        case 'swordman':
          char = new Swordman(elem.character.level);
